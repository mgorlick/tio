...
void stdin_configure(void)
{
    ...
    stdin_new.c_lflag &= ~(ICANON|ISIG|ECHOE|ECHOK|ECHONL);  // MG - leave local ECHO on.
...
void stdout_configure(void)
{
    ...
    // MG. Added this because local-echo doesn't work.
    ...
    while (true)
    {
        ...
        if (status > 0)
        {
            if (FD_ISSET(fd, &rdfs))
            {
		      ...
		      // GRAY on a dark background in nearly invisible so subsitute GREEN.
		      fprintf(stdout, ANSI_COLOR_GREEN "[%s] " ANSI_COLOR_RESET, current_time());
		      next_timestamp = 0;
                    }
